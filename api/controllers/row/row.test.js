...
describe("GET /tables/:tableId/rows/:rowId", () => {
  it("OK, Get detail row", done => {
    request(app)
      .get(`/tables/${tableId}/rows/${listRows[0]._id}`)
      .set({
        "x-access-token": owner.tokenKey
      })
      .then(res => {
...
describe("DELETE /tables/:tableId/rows/:rowId", () => {
  it("OK, delete immediately row", done => {
    request(app)
      .delete(`/tables/${tableId}/rows/${listRows[4]._id}/`)
      .set({
        "x-access-token": owner.tokenKey
      })
      .then(res => {
  ...
  it("OK, check row already deleted ?", done => {
    request(app)
      .get(`/tables/${tableId}/rows/${listRows[4]._id}`)
      .set({
        "x-access-token": owner.tokenKey
      })
      .then(res => {
...
describe("PUT /tables/:tableId/rows/:rowId/", () => {
  it("OK, edit row", done => {
    request(app)
      .put(`/tables/${tableId}/rows/${listRows[0]._id}/`)
      .set({
        "x-access-token": owner.tokenKey
      })
      .send({
