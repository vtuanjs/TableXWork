...
describe('POST /tables/:tableId/rows', () => {
    it('OK, create Row 1', done => {
        request(app).post(`/tables/${tableId}/rows`).set({
...
describe('GET /tables/:tableId/rows', () => {
    it('OK, Query list of rows by table id', done => {
        request(app).get(`/tables/${tableId}/rows`).set({
            'x-access-token': owner.tokenKey
        }).then(res => {
            const body = res.body
            expect(res.statusCode).to.equals(200)
...
describe('GET /tables/:tableId/rows/:rowId', () => {
    it('OK, Get detail row', done => {
        request(app).get(`/tables/${tableId}/rows/${listRows[0]._id}`).set({
            'x-access-token': owner.tokenKey
        }).then(res => {
            const body = res.body
            expect(res.statusCode).to.equals(200)
...
describe('DELETE /tables/:tableId/rows/:rowId', () => {
    it('OK, delete immediately row', done => {
        request(app).delete(`/tables/${tableId}/rows/${listRows[4]._id}/`).set({
            'x-access-token': owner.tokenKey
        }).then(res => {
            const body = res.body
            expect(res.statusCode).to.equals(200)
    ...
    it('OK, check row already deleted ?', done => {
        request(app).get(`/tables/${tableId}/rows/${listRows[4]._id}`).set({
            'x-access-token': owner.tokenKey
        }).then(res => {
            const body = res.body
            expect(res.statusCode).to.equals(400)
...
describe('PUT /tables/:tableId/rows/:rowId/', () => {
    it('OK, edit row', done => {
        request(app).put(`/tables/${tableId}/rows/${listRows[0]._id}/`).set({
            'x-access-token': owner.tokenKey
        }).send({
            title: 'Row Edit',
