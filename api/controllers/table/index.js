...
module.exports.postTable = async (req, res, next) => {
  const { title, description, isAllowMemberAddMember } = req.body;
...
module.exports.updateTable = async (req, res, next) => {
  ...
  try {
    ...
    const table = await Table.findByIdAndUpdate(tableId, query, {
      new: true
    });
 ...
 */
const addRefTableToUsers = ({ tableId, userIds, session }) => {
 ...
 */
const createNotifyJoinTable = ({ message, tableId, userIds, session }) => {
...
const isAllowed = ({ table, idCheck, userCheck }) => {
  if (
    !table.allowed.isAllowMemberAddMember &&
    userCheck.tables.some(item => {
