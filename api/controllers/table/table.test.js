...
describe('POST /tables', () => {
    it('OK, create Table 1', done => {
        request(app).post('/tables').set({
            ...
            expect(body.table.description).to.equals('curae donec pharetra magna vestibulum aliquet ultrices erat tortor sollicitudin mi sit')
            done()
        }).catch((error) => done(error))
    })
    it('OK, create Table 2', done => {
        request(app).post('/tables').set({
            ...
            expect(body.table.description).to.equals('a libero nam dui proin leo odio porttitor id consequat')
            done()
        }).catch((error) => done(error))
    })
    it('OK, create Table 3', done => {
        request(app).post('/tables').set({
            ...
            expect(body.table.description).to.equals('ipsum primis in faucibus orci luctus et ultrices posuere cubilia curae duis faucibus')
            done()
        }).catch((error) => done(error))
    })
    it('OK, create Table 4', done => {
        request(app).post('/tables').set({
            ...
            expect(body.table.description).to.equals('eget eleifend luctus ultricies eu nibh quisque id justo sit amet sapien dignissim')
            done()
        }).catch((error) => done(error))
    })
    it('OK, create Table 5', done => {
        request(app).post('/tables').set({
            ...
            expect(body.table.description).to.equals('vitae consectetuer eget rutrum at lorem integer tincidunt ante vel ipsum praesent blandit lacinia erat vestibulum sed magna at nunc')
            done()
        }).catch((error) => done(error))
    })
...
describe('GET /tables', () => {
    it('OK, Query list of tables', done => {
        request(app).get('/tables?fields=title').set({
            'x-access-token': owner.tokenKey
        }).then(res => {
            const body = res.body
            expect(res.statusCode).to.equals(200)
...
describe('GET /tables/:tableId', () => {
    it('OK, Get detail table', done => {
        request(app).get(`/tables/${listTables[0]._id}`).set({
            'x-access-token': owner.tokenKey
        }).then(res => {
            const body = res.body
            expect(res.statusCode).to.equals(200)
...
describe('POST /tables/:tableId/add-members', () => {
    ...
    it('FAIL, wrong tableId', done => {
        request(app).post(`/tables/5d9468959767571303701cf8/add-members`).set({
            'x-access-token': owner.tokenKey
        }).send({
            userIds: '5d9468959767571303701cf8'
        }).then(res => {
...
describe('POST /tables/:tableId/remove-members', () => {
    it('OK, remove members in table', done => {
        request(app).post(`/tables/${listTables[0]._id}/remove-members`).set({
            'x-access-token': owner.tokenKey
        }).send({
            userIds: userIds[1]
        }).then(res => {
...
describe('GET /tables/:tableId/show-members', () => {
    it('OK, show members in table', done => {
        request(app).get(`/tables/${listTables[0]._id}/show-members`).set({
            'x-access-token': owner.tokenKey
        }).then(res => {
            const body = res.body
            expect(res.statusCode).to.equals(200)
...
describe('POST /tables/:tableId/delete', () => {
    it('OK, send to trash table', done => {
        request(app).post(`/tables/${listTables[0]._id}/delete`).set({
            'x-access-token': owner.tokenKey
        }).then(res => {
            const body = res.body
            expect(res.statusCode).to.equals(200)
...
describe('POST /tables/:tableId/restore', () => {
    it('OK, restore table', done => {
        request(app).post(`/tables/${listTables[0]._id}/restore`).set({
            'x-access-token': owner.tokenKey
        }).then(res => {
            const body = res.body
            expect(res.statusCode).to.equals(200)
...
describe('DELETE /tables/:tableId', () => {
    it('OK, delete immediately table', done => {
        request(app).delete(`/tables/${listTables[4]._id}/`).set({
            'x-access-token': owner.tokenKey
        }).then(res => {
            const body = res.body
            expect(res.statusCode).to.equals(200)
    ...
    it('OK, check table already deleted ?', done => {
        request(app).get(`/tables/${listTables[4]._id}`).set({
            'x-access-token': owner.tokenKey
        }).then(res => {
            const body = res.body
            expect(res.statusCode).to.satisfy(status => {
...
describe('PUT /tables/:tableId/', () => {
    it('OK, edit table, set member can add member', done => {
        request(app).put(`/tables/${listTables[0]._id}/`).set({
            'x-access-token': owner.tokenKey
        }).send({
            title: 'Table Edit',
