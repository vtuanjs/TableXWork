...
describe('POST /job?tableId=', () => {
    it('OK, create Job 1', done => {
        request(app).post(`/job?tableId=${tableId}`).set({
...
describe('GET /job?tableId=', () => {
    it('OK, Query list of jobs by table id', done => {
        request(app).get(`/job?tableId=${tableId}`).set({
            'x-access-token': owner.tokenKey
        }).then(res => {
            const body = res.body
            expect(res.statusCode).to.equals(200)
...
describe('PREPARE ADD MEMBER TO JOB', () => {
    it('Get userIds will add to table', done => {
        // userIds cache from table
        redis.get('userIds').then(data => {
            userIds = JSON.parse(data)
            done()
        }).catch(error => done(error))
    })
    it('Get userId will add to table', done => {
...
describe('GET /job/:tableId/show-members', () => {
    it('OK, show members in table', done => {
        request(app).get(`/tables/${listJobs[0]._id}/show-members`).set({
            'x-access-token': owner.tokenKey
        }).then(res => {
            const body = res.body
            expect(res.statusCode).to.equals(200)
